...
const handleDeleteComment = (commentId) => {
  errorMessage.value = '';
  modalStore.open('Удалить комментарий?', async () => {
...
<template>
    <ul>
      <li v-for="comment in articleStore.article.comments" :key="comment.id" class="bg-white rounded-md shadow-md p-4 mb-4">
        <div class="flex justify-between mb-2">
          ...
          <button v-if="userStore.isAdmin || userStore.isModerator" type="button" class="text-red-500 cursor-pointer hover:text-red-700" @click="handleDeleteComment(comment.id)">
